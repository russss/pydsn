...
class DSN(object):
    ...
    def update(self):
        try:
            if (
                self.last_config_update is None
                or self.last_config_update
                < datetime.now() - timedelta(minutes=self.config_update_interval)
            ):
                self.sites, self.spacecraft = self.parser.fetch_config()
            new_data = self.parser.fetch_data()
        except ConnectionError as e:
            self.log.warning("Unable to fetch data from DSN: %s" % e)
            return
        except LxmlError as e:
            self.log.warning("Unable to parse data: %s", e)
            return
        ...
        for antenna, new_status in new.items():
            ...
            for signal in ("down_signal", "up_signal"):
                if (len(new_status[signal]) > 0 and len(old_status[signal]) == 0) or (
                    len(new_status[signal]) > 0
                    and new_status[signal][0]["debug"] != old_status[signal][0]["debug"]
                ):
                    updated = True
            if updated:
